...
func ResolveSecrets(outputName string, cfg map[string]any, schema output.Schema) error {
	for _, field := range schema.Fields {
		...
		parent, leafKey := navigateMap(cfg, field.Name)
		if parent == nil {
			continue
		}
...
// navigateMap traverses a nested map by dot-separated path segments.
// Returns the parent map containing the leaf key and the leaf key name.
// For "auth.password", navigates into cfg["auth"] and returns (authMap, "password").
// For "password", returns (cfg, "password").
// Returns (nil, "") if any intermediate segment is missing or not a map.
